...
public class LuncherTestView extends ViewGroup {

	private static final String TAG = "LuncherTestView";

	// 存放touch的坐标
	private int[] cellInfo = new int[2];
	private OnLongClickListener mLongClickListener;

	public LuncherTestView(Context context) {
		super(context);
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		for (int index = 0; index < getChildCount(); index++) {
			lp = (LayoutParams) getChildAt(index).getLayoutParams();
			getChildAt(index).layout(lp.x, lp.y, lp.x + lp.width,
					lp.y + lp.height);
		}
	}
